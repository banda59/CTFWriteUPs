...
1. 취약점 식별: `fgets`가 버퍼 크기보다 큰 길이를 읽으므로 스택 오버플로우가 발생한다.
2. 오프셋 산출: 버퍼 64바이트 + saved RBP 8바이트 = 72바이트 패딩 후 RIP를 덮는다.
3. 타깃 함수 선택: `duck()`은 `flag.txt`를 읽어 `FLAG: %s`로 출력하므로 리턴 주소를 `duck`로 바꾼다.
4. 주소 확보: PIE 비활성이므로 정적 심볼 주소 사용이 가능하다. 당신의 출력 기준 `duck = 0x40128c`다.
5. 페이로드 구성: `payload = b"A"*72 + p64(0x40128c)`다.
6. 전송: 원격 서비스 배너 `The Ducks are coming!` 수신 후 페이로드를 보낸다. 성공하면 플래가 나온다.
